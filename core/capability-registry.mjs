...
export const CAPABILITY_DEFINITIONS = [
  {
    ...
    operations: [
      { name: "run", summary: "Execute a DAG workflow of tasks with dependency resolution and concurrency.", params: ["tasks", "concurrency", "rollbackOnError", "timeoutMs", "checkpoint", "runId"] },
      ...
      { name: "resume", summary: "Resume a failed or paused DAG workflow from SQLite checkpoints without repeating completed tasks.", params: ["runId", "fromStep", "patchInput", "concurrency"] },
      { name: "runs", summary: "List recent DAG workflow runs and execution states.", params: ["limit", "status"] },
      { name: "checkpoints", summary: "Inspect step checkpoints and outputs of a workflow run.", params: ["runId"] },
    ],
  ...
  },
  {
    ...
    operations: [
      ...
      { name: "telemetry", summary: "Query ring buffer of recent tool executions, error rates, and gate verification results.", params: ["limit", "tool", "action", "status"] },
    ],
  ...
  },
  {
    ...
    operations: [
      ...
      { name: "gc", summary: "Clean temporary files and stale build artifacts while protecting active capability leased paths.", params: ["path", "directory", "maxAgeHours", "dryRun", "force"] },
    ],
  ...
  },
  {
    ...
    operations: [
      ...
      { name: "grant_lease", summary: "Grant a cryptographic capability lease with call quota and path constraints (no TTL).", params: ["scope", "budget", "allowedPaths", "runId", "taskId", "autoRevoke"] },
      { name: "revoke_lease", summary: "Revoke an active capability lease by ID, runId, or taskId.", params: ["leaseId", "runId", "taskId", "reason"] },
      { name: "list_leases", summary: "List active capability leases and remaining call budgets.", params: ["runId", "taskId", "scope"] },
      { name: "get_lease", summary: "Get details and remaining quota of a specific capability lease.", params: ["leaseId"] },
      { name: "classify_permission_level", summary: "Classify a tool and action into the 4 permission levels (GUEST, USER, POWER_USER, ADMIN).", params: ["tool", "action"] },
      { name: "request_action_approval", summary: "Create an action approval request with unforgeable HMAC signature challenge for user click confirmation.", params: ["tool", "action", "args", "userId"] },
      { name: "approve_user_action", summary: "Validate cryptographic HMAC signature from a user action/click and authorize the operation.", params: ["actionId", "userActionSignature", "clickTimestamp"] },
      { name: "simulate_user_click", summary: "Simulate user action click for deterministic testing and certification gates.", params: ["actionId", "clickTimestamp"] },
      { name: "worm_audit", summary: "Inspect immutable WORM audit log entries recorded by SQLite append-only triggers.", params: ["limit", "action", "tool", "status"] },
      { name: "worm_audit_verify", summary: "Verify cryptographic SHA256 hash chain and trigger immutability of the WORM audit log.", params: [] },
    ],
