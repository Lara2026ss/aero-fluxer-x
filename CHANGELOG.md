...
- **UX de Actualización y Consentimiento Seguro (Zero-Friction)**: Descenso de `upd` a nivel estándar `user` con requisito de confirmación explícita `confirm: true`. Se elimina por completo la fricción de 'auto-escalación de privilegios' en Claude y otros LLMs. Enriquecimiento de las respuestas de autorización (`CONFIRMATION_REQUIRED` y `PERMISSION_DENIED`) explicando con transparencia el Principio de Mínimo Privilegio para proteger la privacidad del usuario sin generar sospechas de malware.
