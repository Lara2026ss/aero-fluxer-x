...
export function createGuideDomain({ runtime, domain }) {
  const actions = {
    ...
    print_guide: async () => ({
      ok: true,
      title: "Guía Maestra: Impresión Inteligente, Vistas Previas y Estilos en Windows (printcenter)",
      overview: "El motor printcenter permite previsualizar documentos con diagramas ASCII y PNGs de alta fidelidad, personalizar estilos de renderizado y escalado, procesar PDFs, archivos de Word (.docx/.doc), imágenes y texto, e imprimir carpetas completas por lotes.",
      tool: "printcenter",
      workflow_steps: [
        {
          step: 1,
          ...
          detail: "Lista todas las impresoras locales, de red USB/WiFi y virtuales (ej: 'Microsoft Print to PDF', 'HP Smart Tank', 'Canon', 'Epson'). Devuelve si está en línea, puerto y estado.",
        },
        {
          step: 2,
          title: "Vista previa y layout (La IA puede 'ver' cómo quedará impreso)",
          command: "printcenter { action: 'preview', file: 'documento.pdf', scale_mode: 'fit_to_printable_area' }",
          detail: "Genera una vista previa fotorrealista en PNG en caché y un diagrama ASCII en la respuesta que muestra la hoja física, los márgenes y la colocación del contenido, informando si hay recorte (clipping_detected) y la cobertura de tinta.",
        },
        {
          step: 3,
          title: "Preflight preventivo con validación de hardware y vista previa",
          command: "printcenter { action: 'preflight', file: 'documento.docx', preview: true }",
          detail: "Valida parámetros contra el controlador de la impresora y opcionalmente adjunta la vista previa visual.",
        },
        {
          step: 4,
          title: "Impresión real de archivo individual o carpeta completa",
          command: "printcenter { action: 'print', file: 'documento.pdf', scale_mode: 'fit_to_printable_area' }",
          detail: "Envía el documento al spooler de impresión de Windows. Si se pasa 'folder' en vez de 'file', procesa e imprime automáticamente todos los archivos compatibles en lote.",
        },
      ],
      render_styles: {
        fit_to_page: "Ajusta el contenido a la hoja física completa manteniendo proporciones y centrado.",
        fit_to_printable_area: "Ajusta el contenido dentro de los márgenes imprimibles del hardware para evitar que la impresora recorte los bordes.",
        actual_size: "Tamaño 100% (escala 1:1) sin alterar cotas milimétricas; avisa si el contenido desborda la hoja.",
        shrink_oversized: "Reduce el tamaño únicamente si sobrepasa la hoja; si cabe, se mantiene al 100%.",
        custom: "Aplica una escala porcentual personalizada vía 'custom_scale' (ej: 85 para 85%, 50 para 50%).",
        stretch_to_fill: "Estira forzando a cubrir toda el área de impresión ignorando el aspect ratio.",
        borderless: "Impresión sin márgenes al borde (ideal para fotos).",
      },
      supported_formats: [
        "PDF (.pdf) — Procesado nativamente vía WinRT a 300-600 DPI",
        "Word (.docx, .doc, .rtf) — Conversión automática e impresión fidedigna",
        "Imágenes (.png, .jpg, .jpeg, .bmp, .webp, .gif, .tiff) — Impresión y vista previa directa",
        "Texto plano (.txt, .md, .log, .json, .csv) — Paginación automática en fuentes monoespaciadas",
      ],
      folder_printing: {
        command: "printcenter { action: 'print', folder: 'C:\\\\Users\\\\...\\\\Documentos' }",
        description: "Escanea la carpeta en busca de PDFs, Word e imágenes compatibles e imprime cada archivo de forma secuencial generando un informe consolidado.",
      },
      parameters: {
        file: "Ruta del archivo a imprimir o previsualizar.",
        folder: "Ruta de la carpeta a imprimir por lote (escanea e imprime todos los archivos compatibles).",
        printer: "Nombre de la impresora destino. Si se omite, se usa la predeterminada.",
        scale_mode: "Estilo de escalado: 'fit_to_page', 'fit_to_printable_area', 'actual_size', 'shrink_oversized', 'custom', 'stretch_to_fill'.",
        custom_scale: "Porcentaje o factor para modo custom (ej: 85 o 0.85).",
        alignment: "Alineación en la página: 'center' o 'top_left'.",
        pages: "Rango de páginas (ej: '1', '1-3', '2,4').",
        copies: "Número de copias.",
        color: "true para Color o false para Monocromático.",
        paper_size: "'Letter', 'A4', 'Legal', 'Tabloid', etc.",
